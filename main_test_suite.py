...
This main_test_suite.py serves as a complete test suite to identify real-world issues
...
class GenieSystemTester:
    ...
    def test_storage_system(self) -> TestResult:
        ...
        try:
            ...
            # The JsonStore doesn't have save_state, it saves automatically when data is modified
            # We'll test by adding a task instead
            from models.task_model import Task, TaskStatus
            test_task = Task(
                heading="Test Task",
                details="This is a test task for storage validation",
                time_estimate=30
            )
            self.store.add_task("test_user", test_task)
            
            # Load data by retrieving the task we just added
            # The task ID might be a UUID string, so we need to convert it
            task_id_str = str(test_task.id)
            loaded_task = self.store.get_task("test_user", task_id_str)
            
            if loaded_task and loaded_task.heading == "Test Task":
    ...
    def test_task_extraction(self) -> TestResult:
        ...
        try:
            ...
            for i, user_input in enumerate(test_inputs):
                try:
                    # Task extraction requires existing_tasks parameter
                    actions = extraction_agent.extract_task(user_input, existing_tasks=[])
                    if actions and len(actions) > 0:
                        # The extract_task returns a list of actions, we'll take the first one
                        action = actions[0]
                        ...
                        if action_type == 'add':
                            # For 'add' action, the task data is directly in the action
                            task_data = {
                            ...
                            logger.info(f"✅ Extracted task {i+1}: {task_data.get('heading', 'Unknown')}")
                        elif action_type == 'edit':
                            # For 'edit' action, we need to handle differently
                            result.add_error(f"Failed to extract task {i+1}: Edit action not supported for new task creation")
                        elif action_type == 'mark_done':
                            # For 'mark_done' action, we need to handle differently
                            result.add_error(f"Failed to extract task {i+1}: Mark done action not supported for new task creation")
                        else:
                            # For any other action type, try to extract task data
                            if action.get('heading'):
                                ...
                                extracted_tasks.append(task_data)
                                logger.info(f"✅ Extracted task {i+1} (type: {action_type}): {task_data.get('heading', 'Unknown')}")
                            else:
                                result.add_error(f"Failed to extract task {i+1}: No heading found in action type '{action_type}'")
                    else:
                        result.add_error(f"Failed to extract task {i+1}: Empty actions")
                except Exception as e:
    ...
    def test_calendar_integration(self) -> TestResult:
        ...
        try:
            ...
            try:
                # Add a small delay to ensure the event is synced
                import time
                time.sleep(2)
    ...
    def test_feedback_processing(self) -> TestResult:
        ...
        try:
            ...
            for i, feedback in enumerate(test_feedback):
                try:
                    # Feedback processing requires current_state parameter
                    current_state = {
                        "user_id": "test_user",
                        "tasks": [],
    ...
    def test_session_management(self) -> TestResult:
        ...
        try:
            ...
            if session and session.user_id == user_id:
                ...
                result.add_data("session_id", str(session.created_at.timestamp()))
            ...
            retrieved_session = self.session_manager.load_session(user_id)
                ...
                try:
                    ...
                    if step == "task_extraction":
                        ...
                        if actions and len(actions) > 0:
                            ...
                            logger.info(f"✅ {step}: Task extracted - {task_heading}")
    ...
    def test_complete_integration(self) -> TestResult:
        ...
        try:
            ...
            result.add_data("task_extracted", task_heading)
